...
> **이 README는 포트폴리오 본문([메인.md](./메인.md))의 보조 문서입니다.**
> 메인 문서는 문제·핵심 의사결정·v1→v2 재설계 흐름·검증 결과를 평가자 관점에서 정리합니다.
> 이 README는 코드 구조·디렉터리·실행 명령·세부 알고리즘 파라미터·DB 스키마 등 구현 디테일을 담습니다.
...
| **완료된 검증** | 48개 운영 세그먼트 확정, 정보부족형 115건 확인, 단건·배치 배정 서비스 동작 검증 |
| **완료된 검증 (추가)** | 8셀 RF hold-out 정확도 99~100% (v2.1-tune 라벨 기준, photo×n3 학습 제외) |
| **아직 안 한 검증** | 고객 반응률 차이(클릭률·지원률), 장기 드리프트 |
...
|---|---|---|
| 분리 기준 | recruitType (model / beauty / photo) | **recruitType × payment_group** (9셀) |
| 속성 추출 | LLM 자유 키워드(Phase1) + LLM 9속성(Phase3) | LLM 6차원 + 1 필터 태그 (카테고리 입력 추가) |
| 차원 발견 | KoSimCSE 임베딩 군집화 (5% 통과 5개로 실패 → top-30 + 도메인 지식) | LLM 유사어 정규화 + 빈도 임계값 실험 + LLM 그룹화 + 마케터 검토 |
| 클러스터링 | 원-핫 + K-means (recruitType별 K-means, 총 26세그) | P3 보정 → C-1 → 가중 Gower(6축) → average linkage(raw K) → dominant 70% 병합 + micro 흡수 |
| 신규 글 배정 | 텍스트 분류기 시도 (TF-IDF Macro F1 37~55% / KLUE-RoBERTa 48~60%, 미채택) | **셀별 RandomForest** (속성 원-핫, hold-out 99~100%) |
| 세그먼트 수 | 26개 (model 12 / beauty 10 / photo 4) | **48개 운영 세그먼트** + 정보부족형 115건 |
| 서비스 형태 | 설계만 (미구현) | CLI(batch/single/stats) + **FastAPI REST API** |
...
v1 단계에서 확인된 3가지 정량 발견이 v2 설계의 직접 근거가 됐습니다.

| 발견 | v1 측정값 | v2 적용 |
|---|---|---|
| 텍스트 직접 분류 vs LLM 속성 경유 격차 | Macro F1 49~60% vs 97~99% (약 50%p) | LLM 운영 유지 + 셀별 RF 채택 |
| 모든 속성 불명확 글이 한 점에 모이는 노이즈 군집 | model 7.3% · beauty 9.7% · photo 24.0% | Gower null-aware + 정보충실도 선분리(C-1) |
| 한국어 표현 분산으로 같은 의미가 3~4개 군집에 흩어짐 | 유니크 키워드 14,141개 → 군집 3,297개 (평균 4.3개), 5% 통과 5개 | LLM 유사어 정규화 전처리 추가 (14,169 → 8,549, 약 40% 축소) |

v1의 5가지 한계(텍스트 분류기 실패·LLM 추출 불명확 50~92%·키워드 맥락 모호성·노이즈 군집 형성·마케터 검토 미수행)는 모두 v2.1-tune에서 구조적으로 처리됐습니다.
...
Task 5  RF 분류기 학습 (8셀, v2.1-tune 기준 hold-out 99~100%)
...
- **RF 학습**: 8셀 학습, 1셀(photo×n3) SKIP. v2.1-tune 라벨 기준 hold-out 정확도 99~100%
